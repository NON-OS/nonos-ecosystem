...
//!
//! Provides resilient Ethereum RPC access with:
...
/// RPC endpoint with health status
#[derive(Clone, Debug)]
...
/// Time to wait before retrying unhealthy endpoint (used in health_check)
#[allow(dead_code)]
const UNHEALTHY_RETRY_SECS: u64 = 60;
...
impl RpcProvider {
    /// Create new provider for Ethereum mainnet
    pub fn mainnet() -> Self {
    ...
    /// Create new provider for Sepolia testnet
    pub fn sepolia() -> Self {
    ...
    /// Create new provider for Base mainnet
    pub fn base() -> Self {
    ...
    /// Create new provider for Arbitrum mainnet
    ...
    /// Add a custom endpoint (e.g., user's own node)
    ...
    /// Get current healthy endpoint URL
    pub async fn get_url(&self) -> NonosResult<String> {
        ...
        for i in 0..len {
            ...
            if endpoint.healthy {
                if i > 0 {
                    // We moved to a different endpoint
        ...
        // All endpoints unhealthy, try the first one anyway
    ...
    /// Report failed request - triggers failover if needed
...
mod tests {
    ...
    fn test_mainnet_endpoints() {
        ...
        // Should have at least 5 endpoints for redundancy
