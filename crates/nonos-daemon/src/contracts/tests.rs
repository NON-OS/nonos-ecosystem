...
fn test_contract_config_chains() {
    ...
    // Verify mainnet uses official NOX token address
...
fn test_nox_token_mainnet_address() {
    ...
    // Verify the address matches the deployed contract
    assert_eq!(
        hex::encode(&addr.0),
...
fn test_contract_client_creation() {
    ...
    // Client is created but not connected - verifying no panic during construction
}
