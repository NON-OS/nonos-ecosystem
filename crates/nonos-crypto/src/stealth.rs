...
impl StealthKeyPair {
    ...
    pub fn derive_from_master(master_key: &nonos_types::Blake3Key) -> NonosResult<Self> {
        // Derive spend key
        let spend_seed = crate::derive_child_key(master_key, &[0x73746561, 0x6b657930]); // "steak0" as u32s
        let spend_private = Secp256k1PrivateKey::from_bytes(spend_seed.0);
        let spend_public = derive_public_key(&spend_private)?;

        // Derive view key
        let view_seed = crate::derive_child_key(master_key, &[0x73746561, 0x6b657931]); // "steak1" as u32s
...
pub fn generate_stealth_address(
    ...
    // Compute shared secret: ECDH(ephemeral_private, view_pubkey)
    let shared_secret = compute_ecdh_shared_secret(&ephemeral_private, &recipient_meta.view_pubkey)?;

    // Derive stealth private key scalar
    ...
    // Compute stealth public key: spend_pubkey + stealth_scalar * G
...
pub fn check_stealth_address(
    ...
    // Compute shared secret: ECDH(view_private, ephemeral_pubkey)
...
pub fn derive_stealth_private_key(
    ...
    // Compute stealth private key: spend_private + stealth_scalar
...
impl StealthScanner {
    ...
    pub fn scan_announcement(
        ...
        // Remember this tag to avoid re-scanning
...
mod tests {
    ...
    fn test_stealth_address_generation_and_scanning() {
        ...
        // Recipient scans and detects the payment
        ...
        // Verify the derived private key produces the same address
    ...
    fn test_stealth_address_not_ours() {
        ...
        // Sender generates stealth address for recipient1
        let (stealth_addr, _) = generate_stealth_address(&recipient1.meta_address()).unwrap();

        // Recipient2 tries to scan - should not match
